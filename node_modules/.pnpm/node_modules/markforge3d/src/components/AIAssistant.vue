<script setup>
// 修复13：引入 clearMessages，提供清空对话历史的入口。
// 修复14（在 DocList 里处理，这里同步风格）：无额外改动。
...
.ai-toggle:hover { transform: scale(1.1) rotate(15deg); box-shadow: 0 6px 20px rgba(0,0,0,0.4); }
.ai-toggle.active { background: #555; transform: rotate(90deg); }

.ai-overlay {
  position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
  background: rgba(0,0,0,0.4); backdrop-filter: blur(2px); z-index: 4998;
}
...
.action-btn:disabled:hover { background: rgba(255,255,255,0.2); color: var(--text-primary); border-color: var(--border-color); }
...
.empty-state {
  ...
  color: var(--text-secondary); font-size: 16px; opacity: 0.8;
}
...
.assistant .bubble {
  background: var(--bg-surface); color: var(--text-primary); border-top-left-radius: 2px;
}
.user .bubble {
  background: var(--color-accent); color: white; border-top-right-radius: 2px;
}
