...
:::</pre>
      ...
      <div v-else-if="type === 'shortcuts'" class="shortcuts">
        <table class="shortcut-table">
          ...
          <tbody>
            <tr><td><kbd>Ctrl</kbd> + <kbd>S</kbd></td><td>保存历史记录</td></tr>
...
.info-header h3 { font-size: 14px; font-weight: 600; color: var(--text-primary); margin: 0; }
.close-btn {
  border: none; background: transparent; font-size: 14px; cursor: pointer;
  color: var(--text-secondary); padding: 4px; border-radius: 4px; transition: all 0.2s;
}
.close-btn:hover { background: var(--bg-hover); color: var(--color-danger); }

.info-content {
  flex: 1; overflow-y: auto; padding: 20px;
  color: var(--text-primary); font-size: 14px; line-height: 1.6;
}
.tutorial .intro { margin-bottom: 20px; color: var(--text-secondary); }
...
.style-list li { padding: 4px 0; border-bottom: 1px dashed var(--border-color); }

pre {
  background: var(--bg-hover); padding: 10px; border-radius: 6px;
  border: 1px solid var(--border-color); overflow-x: auto;
  font-family: 'Fira Code', monospace; font-size: 12px; color: var(--text-primary);
}
code {
...
.shortcut-table th {
  text-align: left; padding: 8px; border-bottom: 2px solid var(--border-color);
  color: var(--text-secondary); font-size: 12px;
}
.shortcut-table td { padding: 8px; border-bottom: 1px solid var(--border-color); font-size: 13px; }
kbd {
